...
See test_report.py for an example.
...
def latex2pdf(filename,pdfname="ABAQUS_RPT.pdf"):
	...
	#os.rename(filename[:-3]+"pdf",pdfname)

def generate_pdf_report(dbpath,pdfname="Report.pdf"):
	...
	PARTS_STR = replace_data(parts_data,PARTS_TEMPLATE,parts_rep)
	INSTANCES_STR = replace_data(instances_data,INSTANCES_TEMPLATE,instances_rep)
	MATERIALS_STR = replace_data(materials_data,MATERIALS_TEMPLATE,materials_rep)
	STEPS_STR = replace_data(steps_data,STEPS_TEMPLATE,steps_rep)
	JOB_STR = replace_data(job_data,JOB_TEMPLATE,job_rep)
	
	CONTENTS_TEMPLATE = tmp.CONTENTS_TEMPLATE.replace("_materialstemplate_",MATERIALS_STR)
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_partstemplate_",PARTS_STR)
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_instancestemplate_",INSTANCES_STR)
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_stepstemplate_",STEPS_STR)
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_jobtemplate_",JOB_STR)
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_deformedshape_","img/deformed_shape.png")
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_vonmises_","img/von_mises.png")
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_pestrain_","img/pe_strain.png")
	CONTENTS_TEMPLATE = CONTENTS_TEMPLATE.replace("_nestrain_","img/ne_strain.png")
	txt = LATEX_TEMPLATE.replace("_contents_",CONTENTS_TEMPLATE)
	
	_temp_latex_file = "__temp.tex"
	f = open(_temp_latex_file,"w")
	f.write(txt)
	f.close()
	latex2pdf(_temp_latex_file,pdfname)
	for _file in glob.glob(_temp_latex_file[:-3]+"*"):
		print "File removed: ",_file
		os.remove(_file)
